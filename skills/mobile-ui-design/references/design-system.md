...
- **The option-selected state** — the one way an *answer* shows it is chosen, on any shape: accent @ 0.12 wash, accent ring (1.5–2pt), and a non-colour channel (a check, or the ring's extra weight). Cards, chips, grid tiles and list rows all use it. insure: region card, cover row, day tile, `ChoiceChip`.
- **Filter chip** (`SelectableChip`) — capsule; selected = solid ink (near-black light / white dark). **Only for choosing what the screen shows** (tabs, list filters). Never for an answer.
- **Choice chip** (`ChoiceChip`) — the answer counterpart: same capsule and padding, selected = the option-selected state with a leading check. Trip purpose, and any single- or multi-choice answer.
- **Choice tile grid** — a 3x3 `LazyVGrid` of equal tiles (48pt min height, `Radius.md`) for picking one value from a short, known set (trip lengths). Selected = the option-selected state. A grid rather than wrapping chips when the values should line up and every target must hold 44pt.
- **Expandable section card** — calm card whose header row (logo, name, one fair-to-compare summary line, chevron) toggles its rows inside the same card; one open at a time; a hairline inset by the card padding between rows; a small micro-caps heading inside when the rows split into groups (one per product). Closed over the customer's choice, the summary line becomes "Сонгосон: …" in accent, so folding never hides the answer.
- **Flag disc / flag stack** — bundled square flag PNG clipped to a circle + 0.5pt hairline; a stack overlaps only when truncated with "+N" (ring in the card's fill between discs), otherwise spaced.
